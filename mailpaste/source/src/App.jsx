...
import { Check, ChevronDown, Clipboard, Copy, FileCode2, Layers3, MousePointer2, RotateCcw, Sparkles, Trash2 } from "lucide-react";
...
function sanitizeHtml(raw) {
  ...
  const doc = new DOMParser().parseFromString(`<!doctype html><html><head></head><body>${cleaned}</body></html>`, "text/html");
...
function renderDocument(raw, annotate = true) {
  ...
  return `<!doctype html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>
    html,body{margin:0;padding:0;min-height:100%;background:#0d0f12;color:#e7e5df;}body{font-family:Arial,Helvetica,sans-serif;}img{max-width:100%;height:auto;}[data-mailpaste-line]{cursor:text;}[data-mailpaste-selected]{outline:2px solid #d5a84e!important;outline-offset:2px;}
...
export default function App() {
  ...
  useEffect(() => {
    const view = new EditorView({
      state: EditorState.create({ doc: initialHtml, extensions: [basicSetup, html(), EditorView.lineWrapping, EditorView.updateListener.of((update) => { if (update.docChanged) updateValue(update.state.doc.toString()); })] }),
  ...
  useEffect(() => () => { clearTimeout(toastTimer.current); clearTimeout(previewTimer.current); }, []);
  ...
  const onPreviewLoad = useCallback(() => {
    ...
    const selectionHandler = () => {
      ...
      if (target?.setAttribute) target.setAttribute("data-mailpaste-selected", "true");
      setSelectedLine(line);
      sourceLine(editorView.current, line);
    };
  ...
  return <TooltipProvider delayDuration={200}>
    <main className="app-shell">
      ...
      <footer className="statusbar"><span><span className="save-dot" /> Saved locally</span><span>Select text in the preview to jump to its source line</span><span className="footer-shortcuts"><kbd>⌘C</kbd> copy <kbd>⌘⇧C</kbd> prompt</span></footer>
